...
# `bed_logic.py` lives in `ai_service/Bed` but models are in `ai_service/Save_file/Bed`.
# Compute the ai_service root and point to the Save_file/Bed folder.
AI_SERVICE_ROOT = os.path.dirname(BASE_DIR)

# --- LOAD TFT MODEL ---
model_path = os.path.join(AI_SERVICE_ROOT, "Save_file", "Bed", "hospital_etu_tft_model_80split.ckpt")
best_tft = None
...
lstm_path = os.path.join(AI_SERVICE_ROOT, "Save_file", "Bed", "hospital_lstm_model.pt")
scaler_path = os.path.join(AI_SERVICE_ROOT, "Save_file", "Bed", "lstm_scaler.pkl")
lstm_model = None
lstm_scaler = None
...
if os.path.exists(lstm_path) and os.path.exists(scaler_path):
    ...
    # torch.load does not accept `weights_only`; load the state dict directly.
    state = torch.load(lstm_path, map_location=torch.device('cpu'))
    # If the saved file contains a state_dict key (common), extract it
...
def get_history():
    try:
        ...
        if record_type == 'SurgeUpdate':
            ...
            if latest:
                return jsonify({
                    "count": latest.get("Surge_Capacity_Available", 0),
    ...
    try:
        ...
        for doc in obs_data:
            ...
            try:
                ...
                label = f"{dt.strftime('%b %d')} ({doc['Shift_ID'][0]})" 
            except:
...
def predict():
    ...
    # 1. Get Real-Time Free Beds (Normal Capacity)
    ...
    # 2. ✅ Get Real-Time Surge Capacity (From DB)
    ...
    # Surge Variables (Upper bounds are dynamically fetched from DB)
    ...
    # Costs: Normal (2) < Surge (10) < External (100)
    # Keeping in ETU (Normal) is prioritized (Cost 1)
    ...
    if total_surge_used > 0: rec_text = "CRITICAL: Activate Corridor C Protocols immediately."
...
# ✅ ONLY START IF RUN DIRECTLY (NOT VIA APP.PY)
if __name__ == '__main__':
    print(f"🚀 Logic loaded directly from: {BASE_DIR}")
